...
if TYPE_CHECKING:
    ...
    from ..config import Target, Scope, InstallConfig
...
class AgentAdapter(ABC):
    ...
    def _extract_managed_region(self, content: str, file_path: Path) -> Optional[str]:
    ...
    def _compute_region_hash(self, content: str, file_path: Path) -> Optional[str]:
        """Compute hash of managed region."""
        from ..markers import compute_region_hash
        return compute_region_hash(content, str(file_path))
    
    def _is_managed_region_modified(self, content: str, file_path: Path, expected_hash: str) -> bool:
        """Check if managed region was modified by user."""
        from ..markers import is_managed_region_modified
        return is_managed_region_modified(content, str(file_path), expected_hash)
    ...
    def _remove_file_record(self, target: str, path: Path) -> bool:
        """Remove a file record from lock state."""
        return self.lock_manager.remove_file(target, path)